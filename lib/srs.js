...
*/
var srs_settings = require('./srs_settings');
    ...
    if (existsSync(arg)) {
        ...
        if (detectJson(arg, file_data)) {
            ...
            if (file.crs && file.crs.properties && file.crs.properties.urn) {
                ...
                return force_wgs84({input:'json file'});
            }
        }
    }
...
function detectJson(arg, file_data) {
    var extension = path.extname(arg).toLowerCase();
    if (extension.indexOf('json') > -1) return true;
    if (file_data[0] == '{') return true;
    var fstring = file_data.toString();
    return (fstring.search('"type"') > -1 &&
        fstring.search('"coordinates"') > -1) ||
        fstring.search('"FeatureCollection"') > -1 ||
        (fstring.search('"geometryType"') > -1 &&
         fstring.search('"esriGeometry"') > -1);
}
